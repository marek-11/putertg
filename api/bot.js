        ...
        if (body.message && body.message.text) {
            ...
            if (userMessage.startsWith('/image')) {
                ...
                try {
                    ...
                    // Normalize Result (It might be an object with .src, or a string)
                    ...
                    if (src.startsWith('http')) {
                        // Case A: It's a standard URL (DALL-E often returns this)
                        finalImage = src; 
                    } else if (src.startsWith('data:image')) {
                        // Case B: It's a Base64 Data URI (Flux/Nano often return this)
                        // Telegram crashes on long strings, so we MUST convert to Buffer
                        const base64Data = src.split(',')[1];
                        finalImage = Buffer.from(base64Data, 'base64');
                    } else {
                        // Case C: Raw Base64 without prefix? Try converting anyway.
                        try {
                    ...
                    // Handle "undefined" error messages by checking the object
                    const errorMessage = e?.message || JSON.stringify(e) || "Unknown error";
                    await bot.sendMessage(chatId, `⚠️ Image failed (${modelName}): ${errorMessage}`);
                }
                return res.status(200).json({});
            }
