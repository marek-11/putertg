...
// --- FIXED IMAGE MODELS ---
// Removed the broken Gemini ID. Using Flux Dev as the "Pro" alternative.
const IMAGE_MODELS = {
    'default': 'black-forest-labs/FLUX.1-schnell',
    '--flux':  'black-forest-labs/FLUX.1-schnell',
    '--dev':   'black-forest-labs/FLUX.1-dev',     // High Quality Flux
    '--pro':   'black-forest-labs/FLUX.1-pro',     // Pro Flux
    '--dalle': 'dall-e-3',
...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        if (body.message && body.message.text) {
            ...
            // --- 4. IMAGE GENERATION (FIXED + AUTO FALLBACK) ---
            if (userMessage.startsWith('/image')) {
                ...
                if (!prompt) {
                    ...
                    await bot.sendMessage(chatId, 
                        ...
                        `*Available Flags:*\n\`${flags}\`\n\n` +
                        `*Example:* \`/image --dev a futuristic city\``, 
                        {parse_mode: 'Markdown'}
                    );
                    return res.status(200).json({});
                }
                ...
                try {
                    ...
                    // If we weren't already using the default, try the default as backup
                    if (selectedModel !== IMAGE_MODELS['default']) {
                        await bot.sendMessage(chatId, `⚠️ ${modelName} failed. Retrying with Flux Default...`);
                        try {
